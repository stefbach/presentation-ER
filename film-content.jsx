...
(function(){
const { Scene, FX, ActTag, Narration, Statement, Photo, Stat, Chip, Bar, Panel, Card3D, Logo, ERmark,
...
function FacesFan(){
  ...
  const startX=(1920-total)/2, baseY=316;
  return (
  ...
  return (
    <div style={{position:'absolute',top:400,left:120,right:120,display:'flex',gap:gap,justifyContent:'center'}}>
      ...
      <div style={{position:'absolute',top:302,left:0,right:0,textAlign:'center',fontFamily:FD,fontWeight:600,fontSize:22,letterSpacing:'0.02em',color:C.blue,opacity:ev(localTime,1.6,0.6)}}>
        Et sur chacun — toujours la même règle : <span style={{color:'#fff',fontWeight:700}}>création de valeur</span>.</div>
    </div>
  );
}
...
{ dur:28, hue:'blue', node:(<React.Fragment>
  ...
  <Narration lines={[
    ...
    'Trente ans à voir comment fonctionnent',
    'vraiment les systèmes de santé —',
    'pas comme les manuels le décrivent.',
...
/* ---------- 3 · ANCRAGE MAURICE ---------- */
{ dur:22, hue:'gold', node:(<React.Fragment>
  <ActTag act="L'engagement" title="Maurice, pour y rester" color={C.gold}/>
  <Photo src={IMG('bach')} x={120} y={250} w={470} h={560} objPos="50% 22%"/>
  <Statement x={660} y={300} size={62} lines={['En 2019,','j’ai choisi Maurice.','Pour y rester.']} accentIdx={[2]} accentColor={C.gold} weight={700}/>
  <Narration lines={[
    'Mon fils est mauricien. Ma femme est mauricienne.',
    'Mes enfants grandiront ici. Cela change tout.',
    ...
    x={660} width={1140} align="left" y={720} size={35} italic={true}
    accent="#F2E6CF" dim="rgba(224,210,180,0.26)" lead={1.4} tail={1.3}/>
</React.Fragment>)},

/* ---------- 4 · L'IA REBAT LES CARTES ---------- */
{ dur:20, hue:'blue', node:(<React.Fragment>
  <Statement align="center" y={280} size={100} weight={700}
    lines={['L’IA rebat','les cartes du jeu.']} accentIdx={[1]} accentColor={C.blue}/>
  <Narration lines={[
    'Les géants du logiciel classique sont lents, lourds —',
    'ils ne savent pas penser en IA.',
    'L’opportunité de Maurice : repartir de zéro,',
    'avec des gens ouverts, agiles, mauriciens.']}
    x={960} width={1500} align="center" y={720} size={40}
    accent="#CFE0FA" dim="rgba(206,219,240,0.22)" lead={1.4} tail={1.2}/>
</React.Fragment>)},

/* ---------- 5 · L'APPROCHE DDS (4 piliers) ---------- */
{ dur:26, hue:'blue', node:(<React.Fragment>
  <ActTag act="L'approche" title="Unique & disruptive"/>
...
{ dur:30, hue:'gold', node:(<React.Fragment>
  <ActTag act="La petite équipe" title="Ce que sept Mauriciens ont bâti" color={C.gold}/>
  <FacesFan/>
  <div style={{position:'absolute',top:672,left:0,right:0,display:'flex',justifyContent:'center',gap:64}}>
    <Stat to={10} prefix="×" suffix="" size={80} color={C.gold} label="Productivité par personne" align="center" at={0.6}/>
    <Stat to={12} prefix="" suffix=" mois" size={80} color={C.green} label="Pour devenir experts" align="center" at={0.8}/>
    <Stat to={0} prefix="" suffix="" size={80} color={C.blue} label="Talent importé" align="center" at={1.0}/>
  </div>
  <Narration lines={[
    'Sept Mauriciens. Aucun n’était ingénieur IA.',
    'Curiosité, envie d’apprendre, foi en l’avenir : j’ai investi en eux.',
    'Formés en douze mois — sept personnes qui, avec leurs agents IA,',
    'ont la force de soixante-dix. L’IA n’est pas une menace pour Maurice.',
    'C’est une opportunité — si l’on investit dans ses gens.']}
    x={960} width={1560} align="center" y={892} size={30} italic={true}
    accent="#F2E6CF" dim="rgba(224,210,180,0.22)" lead={1.6} tail={1.1}/>
</React.Fragment>)},

];

window.SCENES_A = SCENES;
window.FacesFan = FacesFan; window.TimelineViz=TimelineViz; window.Pillars=Pillars;
})();
