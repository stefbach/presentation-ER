...
function EDBmark({h=34, style={}}){
  return (
    <span style={{display:'inline-flex',alignItems:'center',background:'#FFFFFF',borderRadius:Math.round(h*0.28),
      ...
      <img src="assets/logos/edb.svg" alt="Economic Development Board — Mauritius" style={{height:h,width:'auto',display:'block'}}/>
    </span>
  );
}
  ...
  return (
    <div style={{position:'absolute',inset:0,pointerEvents:'none',opacity:op}}>
      ...
      <div style={{position:'absolute',top:44,right:120,display:'flex',alignItems:'center',gap:14,opacity:0.85}}>
        <span style={{fontFamily:FD,fontWeight:500,fontSize:14,letterSpacing:'0.16em',textTransform:'uppercase',color:C.dim}}>{window.FILM_LANG==='en'?'Presented to the':'Présenté à l’'}</span>
        <EDBmark h={22}/>
      </div>
    </div>
  );
...
Object.assign(window, { Scene, FX, ActTag, Narration, Statement, Photo, Stat, Chip, Bar, Panel, Card3D, Hud, Logo, Swan, PhotoTile, FlowSvg, FlowLink, NodeChip, Phone, EDBmark });
})();
