...
function TimelineViz(){
  ...
  const ph=[['1993 — 2003','Hospital physician (France)','The public system, from the inside'],
    ['2003 — 2008','Clinic founder','Operational & financial management'],
    ['2008 — 2019','Health economist','Le Figaro ranking · 30+ publications · 2 exits'],
    ['2019 — 2026','Mauritius','DDS founder · AI since the LLMs arrived · GPT-3, 2020']];
...
function Pillars(){
  const {localTime}=useScene();
  const cols=[
    ['AI SaaS','Software in production, not demos. In real daily use.',C.blue],
    ['AI agents','Autonomous agents that do the work — voice, admin, decisions.',C.teal],
    ['Domain expertise','Real know-how on every product: health, accounting, HR, customer care.',C.gold],
    ['LLM mastery','Every model mastered — the right model in the right place, at the lowest cost.',C.green]];
  const W=402,gap=20;
  return (
    <div style={{position:'absolute',top:400,left:120,right:120,display:'flex',gap:gap,justifyContent:'center'}}>
      ...
      <div style={{position:'absolute',top:302,left:0,right:0,textAlign:'center',fontFamily:FD,fontWeight:600,fontSize:22,letterSpacing:'0.02em',color:C.blue,opacity:ev(localTime,1.6,0.6)}}>
        Seven Mauritians trained in 12 months · the strength of 70 with their AI agents · <span style={{color:'#fff',fontWeight:700}}>0 imported talent</span></div>
    </div>
  );
}
...
function DiabetesStakes(){
  const {localTime}=useScene();
  const cards=[
    ['1 in 5','Mauritian adults is diabetic',C.coral,null],
    ['1 in 3',"doesn't know it yet",C.gold,null],
    [null,'deaths caused by diabetes in 2024 — nearly one in four',C.blue,2709]];
  return (
...
{ dur:15, hue:'blue', node:(<React.Fragment>
  ...
  <div style={{position:'absolute',left:130,bottom:54,display:'flex',alignItems:'center',gap:16}}>
    <span style={{fontFamily:FD,fontWeight:600,fontSize:14,letterSpacing:'0.14em',textTransform:'uppercase',color:'rgba(206,219,240,0.55)'}}>Incubated at</span>
    ...
    <span style={{fontFamily:FD,fontWeight:600,fontSize:14,letterSpacing:'0.14em',textTransform:'uppercase',color:'rgba(206,219,240,0.55)'}}>Backed by</span>
...
{ dur:30, hue:'blue', node:(<React.Fragment>
  <ActTag act="The founder" title="Dr Stéphane Bach"/>
  <TimelineViz/>
  <Narration lines={[
    'My name is Stéphane Bach. I am a physician —',
    'a public-health specialist. Thirty years of seeing',
    'how healthcare systems really work.',
    'I have run clinics, published algorithms',
    'with Aix-Marseille University, completed two exits.',
    'In 2019, I chose Mauritius — for good.',
    'My wife is Mauritian; my children will grow up here.',
...
{ dur:35, hue:'blue', node:(<React.Fragment>
  <ActTag act="DDS" title="An AI software producer"/>
  <Statement x={120} y={150} size={54} weight={700}
    lines={['DDS is not an AI agency.','It is an AI software producer.']} accentIdx={[1]} accentColor={C.blue}/>
  <div style={{position:'absolute',top:316,left:0,right:0,display:'flex',justifyContent:'center',alignItems:'center',gap:44}}>
    ...
    <span style={{fontFamily:FD,fontWeight:600,fontSize:20,color:C.dim}}>3 AI products in production</span>
  </div>
  <Pillars/>
  <Narration lines={[
    'Three products in production — TIBOK, Lexora, Axon —',
...
{ dur:30, hue:'coral', node:(<React.Fragment>
  ...
  <Statement x={120} y={160} size={56} weight={700}
    lines={['While the body stays silent,','medicine arrives too late.']} accentIdx={[1]} accentColor={C.coral}/>
  <DiabetesStakes/>
  <Narration lines={[
