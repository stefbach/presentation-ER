...
function Movie(){
  ...
  const PAD = 1.0;  // breath after each narration before the crossfade
  let cursor = 0;
  const placed = scenes.map((s,i)=>{
    const vo = window.VO && window.VO[i];
    const dur = vo ? vo.dur + PAD : s.dur;   // retime each scene to its voice-over
    const start = cursor;
    const end = start + dur;
    cursor = end - OV;
...
// French voice-over (ElevenLabs). Plays the per-scene clip in sync with the
...
function VoiceOver({placed}){
  ...
  React.useEffect(()=>{
    ...
    // audio is the master: resync only on a real jump (scrubber seek), not on small drift, to avoid clipping words
  ...
  return (
    <div onClick={()=>setMuted(m=>!m)} title="Activer / couper la voix off (M)"
      style={{position:'absolute',right:48,bottom:128,zIndex:60,pointerEvents:'auto',cursor:'pointer',
        ...
        boxShadow:muted?'none':'0 0 18px '+C.blue+'66',backdropFilter:'blur(4px)',
        fontFamily:FD,fontWeight:600,fontSize:18,color:muted?'rgba(220,230,245,0.7)':'#fff',userSelect:'none'}}>
